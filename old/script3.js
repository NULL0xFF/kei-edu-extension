function getSyncMemberData(mailAddress = "") {
...
function getSyncCourseList(memberData) {
    var $form = jQuery("#memberSearchForm");
    var result = "";
    var data = new Object();
...
async function massiveSyncSearchCourseByEmail(chunk = 1000, p = 0, q = 0) {
    ...
    for (p; p <= Math.min((mailArray.length / chunk), q); p++) {
        await bulkSyncSearchCourseByMail(mailArray.slice(p * chunk, (p + 1) * chunk), p);
        await sleep(60000);
    }
}

async function bulkSyncSearchCourseByMail(mailArray = [], fileName = "") {
    ...
    new Promise(function (resolve, reject) {
        ...
        mailArray.forEach(function (mailAddress, mailArrayIndex, mailArray) {
            new Promise(function (resolve, reject) {
                ...
                getSyncMemberData(mailAddress).then(function (memberData) {
                    console.log("resolving course");
                    getSyncCourseList(memberData).then(courseList => {
                        var user = {
                            member: memberData,
