 ...
 */
class Completion {
    constructor(csMemberSeq, csMemberId, csMemberName, cxMemberEmail, csApplyStatusCd, csCompletionYn, cxCompletionDate) {
    ...
    formatCompletionDate(cxCompletionDate) {
        ...
        // Formatted Date is in format: yyyy-MM-dd with Date object type.
        // Becareful that original date value is offset with local timezone.
        ...
        const formattedDate = new Date(Date.UTC(year, month - 1, day, hour, minute, second) + timezoneOffset);
        // console.debug(`Original Date: ${cxCompletionDate}`);
...
class Course {
    ...
    constructor(csCourseActiveSeq, csCourseMasterSeq, csTitle, csStatusCd, csCourseTypeCd, csYear, csApplyStartDate, csApplyEndDate, csStudyStartDate, csStudyEndDate, csOpenStartDate, csOpenEndDate, csCmplTime, csTitlePath, csCmplList = []) {
 ...
 * @throws {Error} - The error that occurred while fetching the completion records.
 */
function getCourseCompletion(csCourseActiveSeq, count) {
    return new Promise((resolve, reject) => {
        jQuery.ajax({
            url: "/course/cmpl/selectCmplList.do",
            type: "post",
            ...
            retryLimit: 3,
            success: function (data) {
                resolve(data.list.map(completion => new Completion(
...
async function fetchCourses(action) {
    ...
    for (let i = 0; i < courses.length; i++) {
        ...
        console.log(`Processing course [${i + 1} / ${courses.length}] ${course.csYear} ${course.csTitle}...`);

        console.debug(`Fetching class count for course ${course.csCourseActiveSeq}...`)
        var classCount = await getCourseClassCount(course.csCourseActiveSeq);
        var examCount = await getCourseExamCount(course);
        console.debug(`Found ${classCount} classes for course ${course.csCourseActiveSeq}.`)
        console.debug(`Found ${examCount} exams for course ${course.csCourseActiveSeq}.`)
        course.csCmplTime = classCount + examCount;

        console.debug(`Fetching completion count for course ${course.csCourseActiveSeq}...`)
        ...
        console.debug(`Fetching completions for course ${course.csCourseActiveSeq}...`)
        const completions = await getCourseCompletion(course.csCourseActiveSeq, completionCount);
        console.debug(`Fetched ${completions.length} completions for course ${course.csCourseActiveSeq}.`)
